import { DeliveryRequest } from './requestTypes';

// Normal requests always have a full Monday–Sunday delivery window (selected
// via the WeekPicker); emergency requests (kind: 'emergency') are always a
// single day, so startDate === endDate for those. dailySlots only needs
// entries for the days that actually have slots, everything else in the
// window defaults to 0.
export const MOCK_REQUESTS: DeliveryRequest[] = [
  {
    id: 'REQ-001', contractorName: 'Balfour Beatty', kind: 'normal',
    startDate: '2026-06-22', endDate: '2026-06-28',
    dailySlots: {
      '2026-06-22': { inbound: 3, outbound: 2, twoWay: 1 },
      '2026-06-23': { inbound: 2, outbound: 1, twoWay: 0 },
      '2026-06-24': { inbound: 1, outbound: 0, twoWay: 1 },
      '2026-06-25': { inbound: 0, outbound: 2, twoWay: 0 },
      '2026-06-26': { inbound: 2, outbound: 0, twoWay: 1 },
      '2026-06-27': { inbound: 1, outbound: 1, twoWay: 0 },
      '2026-06-28': { inbound: 0, outbound: 1, twoWay: 0 },
    },
    allocatedSlots: {
      '2026-06-22': { inbound: 3, outbound: 2, twoWay: 1 },
      '2026-06-23': { inbound: 2, outbound: 1, twoWay: 0 },
      '2026-06-24': { inbound: 1, outbound: 0, twoWay: 1 },
      '2026-06-25': { inbound: 0, outbound: 2, twoWay: 0 },
      '2026-06-26': { inbound: 2, outbound: 0, twoWay: 1 },
      '2026-06-27': { inbound: 1, outbound: 1, twoWay: 0 },
      '2026-06-28': { inbound: 0, outbound: 1, twoWay: 0 },
    },
    bookedSlots: {
      '2026-06-22': { inbound: 1, outbound: 1, twoWay: 0 },
      '2026-06-23': { inbound: 1, outbound: 0, twoWay: 0 },
      '2026-06-24': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-25': { inbound: 0, outbound: 1, twoWay: 0 },
      '2026-06-26': { inbound: 1, outbound: 0, twoWay: 0 },
      '2026-06-27': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-28': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    vehicleType: 'HGV_ACA_MDS', driverRoute: 'route1a',
    status: 'approved', submittedAt: '2026-06-20T10:30:00Z',
    notes: 'Morning delivery preferred — before 10:00 AM.',
  },
  {
    id: 'REQ-002', contractorName: 'Kier Group', kind: 'emergency',
    startDate: '2026-06-24', endDate: '2026-06-24',
    dailySlots: {
      '2026-06-24': { inbound: 2, outbound: 0, twoWay: 1 },
    },
    allocatedSlots: {
      '2026-06-24': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    bookedSlots: {
      '2026-06-24': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    vehicleType: 'HDV_MDS', driverRoute: 'route1a',
    status: 'pending', submittedAt: '2026-06-23T08:15:00Z',
    notes: '',
  },
  {
    id: 'REQ-007', contractorName: 'Skanska UK', kind: 'normal',
    startDate: '2026-06-22', endDate: '2026-06-28',
    dailySlots: {
      '2026-06-22': { inbound: 1, outbound: 0, twoWay: 0 },
      '2026-06-23': { inbound: 0, outbound: 1, twoWay: 0 },
      '2026-06-24': { inbound: 2, outbound: 1, twoWay: 0 },
      '2026-06-25': { inbound: 1, outbound: 0, twoWay: 1 },
      '2026-06-26': { inbound: 0, outbound: 2, twoWay: 0 },
      '2026-06-27': { inbound: 1, outbound: 1, twoWay: 0 },
      '2026-06-28': { inbound: 0, outbound: 0, twoWay: 1 },
    },
    allocatedSlots: {
      '2026-06-22': { inbound: 1, outbound: 0, twoWay: 0 },
      '2026-06-23': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-24': { inbound: 1, outbound: 1, twoWay: 0 },
      '2026-06-25': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-26': { inbound: 0, outbound: 1, twoWay: 0 },
      '2026-06-27': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-28': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    bookedSlots: {
      '2026-06-22': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-23': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-24': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-25': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-26': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-27': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-28': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    vehicleType: 'LGV_MDS', driverRoute: 'route2a',
    status: 'pending', submittedAt: '2026-06-20T09:00:00Z',
    notes: 'Partial allocation confirmed — remaining slots to follow.',
  },
  {
    id: 'REQ-008', contractorName: 'Morgan Sindall', kind: 'emergency',
    startDate: '2026-06-25', endDate: '2026-06-25',
    dailySlots: {
      '2026-06-25': { inbound: 0, outbound: 3, twoWay: 1 },
    },
    allocatedSlots: {
      '2026-06-25': { inbound: 0, outbound: 3, twoWay: 1 },
    },
    bookedSlots: {
      '2026-06-25': { inbound: 0, outbound: 1, twoWay: 0 },
    },
    vehicleType: 'HDV_MDS', driverRoute: 'route1a',
    status: 'approved', submittedAt: '2026-06-24T13:20:00Z',
    notes: '',
  },
  {
    id: 'REQ-009', contractorName: 'Wates Group', kind: 'normal',
    startDate: '2026-06-22', endDate: '2026-06-28',
    dailySlots: {
      '2026-06-22': { inbound: 1, outbound: 0, twoWay: 0 },
      '2026-06-23': { inbound: 0, outbound: 1, twoWay: 1 },
      '2026-06-24': { inbound: 1, outbound: 1, twoWay: 0 },
      '2026-06-25': { inbound: 0, outbound: 0, twoWay: 2 },
      '2026-06-26': { inbound: 1, outbound: 0, twoWay: 2 },
      '2026-06-27': { inbound: 0, outbound: 2, twoWay: 0 },
      '2026-06-28': { inbound: 1, outbound: 0, twoWay: 0 },
    },
    allocatedSlots: {
      '2026-06-22': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-23': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-24': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-25': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-26': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-27': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-28': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    bookedSlots: {
      '2026-06-22': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-23': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-24': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-25': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-26': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-27': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-28': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    vehicleType: 'HGV_ACA_MDS', driverRoute: 'route3a',
    status: 'pending', submittedAt: '2026-06-21T10:10:00Z',
    notes: 'Awaiting allocation confirmation.',
  },
  {
    id: 'REQ-010', contractorName: 'Galliford Try', kind: 'normal',
    startDate: '2026-06-22', endDate: '2026-06-28',
    dailySlots: {
      '2026-06-22': { inbound: 1, outbound: 1, twoWay: 0 },
      '2026-06-23': { inbound: 0, outbound: 2, twoWay: 0 },
      '2026-06-24': { inbound: 2, outbound: 0, twoWay: 1 },
      '2026-06-25': { inbound: 1, outbound: 1, twoWay: 0 },
      '2026-06-26': { inbound: 0, outbound: 2, twoWay: 1 },
      '2026-06-27': { inbound: 2, outbound: 2, twoWay: 0 },
      '2026-06-28': { inbound: 1, outbound: 0, twoWay: 0 },
    },
    allocatedSlots: {
      '2026-06-22': { inbound: 1, outbound: 1, twoWay: 0 },
      '2026-06-23': { inbound: 0, outbound: 2, twoWay: 0 },
      '2026-06-24': { inbound: 2, outbound: 0, twoWay: 1 },
      '2026-06-25': { inbound: 1, outbound: 1, twoWay: 0 },
      '2026-06-26': { inbound: 0, outbound: 2, twoWay: 1 },
      '2026-06-27': { inbound: 2, outbound: 1, twoWay: 0 },
      '2026-06-28': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    bookedSlots: {
      '2026-06-22': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-23': { inbound: 0, outbound: 1, twoWay: 0 },
      '2026-06-24': { inbound: 1, outbound: 0, twoWay: 0 },
      '2026-06-25': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-06-26': { inbound: 0, outbound: 1, twoWay: 0 },
      '2026-06-27': { inbound: 1, outbound: 0, twoWay: 0 },
      '2026-06-28': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    vehicleType: 'HGV_ACA_MDS', driverRoute: 'route1a',
    status: 'approved', submittedAt: '2026-06-21T15:45:00Z',
    notes: '',
  },
  {
    id: 'REQ-011', contractorName: 'Sir Robert McAlpine', kind: 'emergency',
    startDate: '2026-06-27', endDate: '2026-06-27',
    dailySlots: {
      '2026-06-27': { inbound: 0, outbound: 0, twoWay: 1 },
    },
    allocatedSlots: {
      '2026-06-27': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    bookedSlots: {
      '2026-06-27': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    vehicleType: 'LGV_MDS', driverRoute: 'route2a',
    status: 'rejected', submittedAt: '2026-06-26T07:30:00Z',
    notes: 'Weekend delivery declined — resubmit for a weekday.',
  },
  {
    id: 'REQ-003', contractorName: 'Laing O\'Rourke', kind: 'normal',
    startDate: '2026-06-29', endDate: '2026-07-05',
    dailySlots: {
      '2026-06-30': { inbound: 2, outbound: 2, twoWay: 2 },
      '2026-07-01': { inbound: 1, outbound: 1, twoWay: 0 },
    },
    allocatedSlots: {
      '2026-06-30': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-07-01': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    bookedSlots: {
      '2026-06-30': { inbound: 0, outbound: 0, twoWay: 0 },
      '2026-07-01': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    vehicleType: 'HGV_ACA_MDS', driverRoute: 'route2a',
    status: 'pending', submittedAt: '2026-06-22T14:00:00Z',
    notes: 'Two separate drops required.',
  },
  {
    id: 'REQ-004', contractorName: 'Bouygues UK', kind: 'emergency',
    startDate: '2026-07-02', endDate: '2026-07-02',
    dailySlots: { '2026-07-02': { inbound: 5, outbound: 0, twoWay: 0 } },
    allocatedSlots: { '2026-07-02': { inbound: 0, outbound: 0, twoWay: 0 } },
    bookedSlots: { '2026-07-02': { inbound: 0, outbound: 0, twoWay: 0 } },
    vehicleType: 'LGV_MDS', driverRoute: 'route1a',
    status: 'rejected', submittedAt: '2026-07-01T09:45:00Z',
    notes: 'Slot unavailable — please resubmit for 03 July.',
  },
  {
    id: 'REQ-005', contractorName: 'Costain Group', kind: 'normal',
    startDate: '2026-06-29', endDate: '2026-07-05',
    dailySlots: { '2026-07-03': { inbound: 0, outbound: 3, twoWay: 1 } },
    allocatedSlots: { '2026-07-03': { inbound: 0, outbound: 3, twoWay: 1 } },
    bookedSlots: { '2026-07-03': { inbound: 0, outbound: 1, twoWay: 0 } },
    vehicleType: 'HGV_ACA_MDS', driverRoute: 'route3a',
    status: 'approved', submittedAt: '2026-06-24T11:00:00Z',
    notes: '',
  },
  {
    id: 'REQ-006', contractorName: 'Mace Group', kind: 'normal',
    startDate: '2026-07-06', endDate: '2026-07-12',
    dailySlots: { '2026-07-06': { inbound: 2, outbound: 1, twoWay: 0 } },
    allocatedSlots: { '2026-07-06': { inbound: 0, outbound: 0, twoWay: 0 } },
    bookedSlots: { '2026-07-06': { inbound: 0, outbound: 0, twoWay: 0 } },
    vehicleType: 'HGV_ACA_MDS', driverRoute: 'route2a',
    status: 'pending', submittedAt: '2026-06-25T16:20:00Z',
    notes: 'Follow-up delivery for the following week.',
  },
  // Additional emergency requests sharing dates with REQ-002 and REQ-008 —
  // demonstrates the merged Delivery Date / Available Slot cells on the
  // FMT Emergency Requests listing when several requests land on one day.
  {
    id: 'REQ-012', contractorName: 'Volker Construction', kind: 'emergency',
    startDate: '2026-06-24', endDate: '2026-06-24',
    dailySlots: {
      '2026-06-24': { inbound: 1, outbound: 1, twoWay: 0 },
    },
    allocatedSlots: {
      '2026-06-24': { inbound: 1, outbound: 1, twoWay: 0 },
    },
    bookedSlots: {
      '2026-06-24': { inbound: 0, outbound: 1, twoWay: 0 },
    },
    vehicleType: 'LGV_MDS', driverRoute: 'route2a',
    status: 'approved', submittedAt: '2026-06-23T09:05:00Z',
    notes: 'Second emergency drop for the same day — separate contractor.',
  },
  {
    id: 'REQ-013', contractorName: 'Interserve', kind: 'emergency',
    startDate: '2026-06-25', endDate: '2026-06-25',
    dailySlots: {
      '2026-06-25': { inbound: 2, outbound: 0, twoWay: 0 },
    },
    allocatedSlots: {
      '2026-06-25': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    bookedSlots: {
      '2026-06-25': { inbound: 0, outbound: 0, twoWay: 0 },
    },
    vehicleType: 'HGV_ACA_MDS', driverRoute: 'route3a',
    status: 'pending', submittedAt: '2026-06-24T11:40:00Z',
    notes: 'Awaiting FMT allocation.',
  },
  {
    id: 'REQ-014', contractorName: 'BAM Nuttall', kind: 'emergency',
    startDate: '2026-06-25', endDate: '2026-06-25',
    dailySlots: {
      '2026-06-25': { inbound: 0, outbound: 1, twoWay: 1 },
    },
    allocatedSlots: {
      '2026-06-25': { inbound: 0, outbound: 1, twoWay: 1 },
    },
    bookedSlots: {
      '2026-06-25': { inbound: 0, outbound: 0, twoWay: 1 },
    },
    vehicleType: 'HDV_MDS', driverRoute: 'route1a',
    status: 'approved', submittedAt: '2026-06-24T15:20:00Z',
    notes: 'Third emergency drop for the same day.',
  },
];
