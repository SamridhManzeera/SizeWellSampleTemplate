...
import {
  ...
  HOURS,
  DayConfig,
  DCO_HOUR_CONSTRAINTS,
} from './ScheduleConfigContext';
...
function getSlotsSummary(config: DayConfig): {
  total: number;
  assigned: number;
  ...
  const utilization =
    config.totalCapacity > 0 ? assigned / config.totalCapacity : 0;
...
function ScheduleConfig() {
  ...
  const [draft, setDraft] = useState<DayConfig>(() =>
    getConfigForDate(todayString())
  );
  ...
  const [activeRangeType, setActiveRangeType] = useState<
    'thisWeek' | 'nextWeek' | 'custom' | null
  >(null);
  ...
  const [copyModalTab, setCopyModalTab] = useState<'single' | 'pattern'>(
    'single'
  );
  ...
  const [modalPatternUntil, setModalPatternUntil] = useState(() =>
  ...
  function handleModalApplyCopy() {
    ...
    applyConfigCopies(
      targetDates.map((date) => ({ date, config: sourceConfig }))
    );
  ...
  const patternTargetDates = getDatesInRange(
    patternMinUntil,
    modalPatternUntil
  );
  ...
  const rangeTotalSlots = weekTableDates.reduce(
    (sum, d) => sum + getMockDailyCapacity(d),
    0
  );
  const rangeAssignedSlots = weekTableDates.reduce(
    (sum, d) => sum + getMockDailyTotal(d),
    0
  );
  const rangeSummaryTitle =
    activeRangeType === 'custom' ? 'Range Summary' : 'Week Summary';
  const rangeSummarySubtitle =
    weekTableDates.length > 0
      ? `${formatDateDisplay(weekTableDates[0])} – ${formatDateDisplay(
          weekTableDates[weekTableDates.length - 1]
        )}`
      : '';

  const copyToDatesCount = getDatesInRange(
    modalCopyToStart,
    modalCopyToEnd
  ).length;
  ...
  function setHourLimit(h: number, v: number) {
    setDraft((prev) => ({
      ...prev,
      hourLimits: { ...prev.hourLimits, [h]: v },
    }));
    setSaved(false);
  }

  function isBlocked(h: number) {
    return (draft.hourLimits[h] ?? 0) === -1;
  }
  ...
  return (
    <div className="sc">
      ...
      <PageHero
        ...
        subtitle="Configure slot capacities and delivery limits per date"
        eyebrow={null}
        actions={
          <div className="sc__hero-actions">
            ...
            >
              <svg
                ...
                className="sc__copy-trigger-icon"
              >
      ...
      <div className="sc__body">
        ...
        <div className="sc__toolbar">
          ...
          <button
            type="button"
            className={`sc__today-btn${
              isToday && viewMode === 'day' ? ' sc__today-btn--active' : ''
            }`}
          ...
          <button
            type="button"
            className={`sc__today-btn${
              activeRangeType === 'thisWeek' ? ' sc__today-btn--active' : ''
            }`}
          ...
          <button
            type="button"
            className={`sc__today-btn${
              activeRangeType === 'nextWeek' ? ' sc__today-btn--active' : ''
            }`}
          ...
          <div className="sc__range-btn-wrap">
            <button
              type="button"
              className={`sc__today-btn${
                activeRangeType === 'custom' ? ' sc__today-btn--active' : ''
              }`}
            ...
            {isRangePickerOpen && (
              <>
                ...
                <div className="sc__range-popover">
                  ...
                  <button
                    type="button"
                    className="sc__range-popover-apply"
          ...
          <div className="sc__week-block">
            ...
            <div className="sc__hour-summary">
              ...
              <div className="sc__hour-summary-item">
                ...
                <span className="sc__hour-summary-val sc__hour-summary-val--used">
                  {daySummary.assigned}
                </span>
              </div>
              <div className="sc__hour-summary-sep" />
              <div className="sc__hour-summary-item">
                ...
                <span className="sc__hour-summary-val sc__hour-summary-val--avail">
                  {daySummary.total - daySummary.assigned}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="sc__week-block">
            ...
            <div className="sc__hour-summary">
              <div className="sc__hour-summary-item">
                ...
                <span className="sc__hour-summary-val">
                  {weekTableDates.length}
                </span>
              </div>
              <div className="sc__hour-summary-sep" />
              <div className="sc__hour-summary-item">
          ...
          <>
            ...
            <section className="sc__section">
              <div className="sc__section-header">
                ...
                <p className="sc__section-desc">
                  Total deliveries allowed for {formatDateDisplay(selectedDate)}
                </p>
              </div>

              <div className="sc__total-slot-card">
                <div className="sc__total-slot-left">
                  <div className="sc__total-slot-icon">
                    <TotalSlotsIcon />
                  </div>
                  <div>
                    <span className="sc__total-slot-label">Total Slots</span>
                    <span className="sc__total-slot-desc">
                      Maximum deliveries for the day
                    </span>
                  </div>
                </div>
                <NumberInput
                  ...
                  min={1}
                  onChange={(v) => {
                    setDraft((prev) => ({ ...prev, totalCapacity: v }));
                    setSaved(false);
                  }}
                />
              </div>
                  ...
                  <div key={ci} className="sc__hour-col">
                    {col.map((h) => {
                      ...
                      return (
                        <div
                          key={h}
                          className={`sc__hour-row${
                            blocked ? ' sc__hour-row--blocked' : ''
                          }`}
                        >
                          ...
                          <div className="sc__hour-type-wrap">
                            <button
                              type="button"
                              className={`sc__hour-type-btn${
                                hType ? ` sc__hour-type-btn--${hType}` : ''
                              }`}
                              onClick={() =>
                                setOpenHourMenu(menuOpen ? null : h)
                              }
                            >
                              {hType === 'peak'
                                ? 'Peak Hour'
                            ...
                            {menuOpen && (
                              <div className="sc__hour-type-menu">
                                <button
                                  type="button"
                                  className="sc__hour-type-opt"
                                  onClick={() => setHourType(h, null)}
                                >
                                  Normal
                                </button>
                                <button
                                  type="button"
                                  className="sc__hour-type-opt sc__hour-type-opt--shoulder"
                                  onClick={() => setHourType(h, 'shoulder')}
                                >
                                  Shoulder Hour
                                </button>
                                <button
                                  type="button"
                                  className="sc__hour-type-opt sc__hour-type-opt--peak"
                                  onClick={() => setHourType(h, 'peak')}
                                >
                                  Peak Hour
                                </button>
                              </div>
                          ...
                          <NumberInput
                            value={blocked ? 0 : val}
                            onChange={(v) =>
                              setHourLimit(h, Math.min(v, totalCapacity))
                            }
                            min={0}
                            max={totalCapacity}
                            disabled={blocked}
                          ...
                          <div className="sc__hour-btns">
                            <button
                              type="button"
                              className={`sc__hour-btn sc__hour-btn--block${
                                blocked ? ' sc__hour-btn--active' : ''
                              }`}
        ...
        {viewMode === 'range' && (
          <section className="sc__section">
            <div className="sc__week-table-header">
              <div className="sc__total-slot-left">
                ...
                <div>
                  ...
                  <span className="sc__total-slot-desc">
                    Per-hour limits across the selected dates
                  </span>
                </div>
              </div>
            </div>
                ...
                <tbody>
                  {WEEK_TABLE_HOUR_GROUPS.map((group, gi) => (
                    <Fragment key={group.label}>
                      <tr
                        className="sc__week-table-group-row"
                        onClick={() => toggleWeekTableGroup(gi)}
                      >
                        <td colSpan={weekTableDates.length + 1}>
                          <div className="sc__week-table-group-inner">
                            <span
                              className={`sc__week-table-group-chevron${
                                expandedGroups[gi]
                                  ? ''
                                  : ' sc__week-table-group-chevron--collapsed'
                              }`}
                            >
                              ▾
                            </span>
                            <span className="sc__week-table-group-label">
                              {group.label}
                            </span>
                            <span className="sc__week-table-group-count">
                              {group.hours.length} hourly slots
                            </span>
                          </div>
                        </td>
                      </tr>
                      {expandedGroups[gi] &&
                        group.hours.map((h) => (
                          <tr key={h} className="sc__week-table-row">
                            <td className="sc__week-table-time-col">
                            ...
                            {weekTableDates.map((date) => {
                              ...
                              return (
                                <td key={date}>
                                  <span
                                    className={`sc__week-table-cell${
                                      val === 0
                                        ? ' sc__week-table-cell--empty'
                                        : ''
                                    }`}
                                  >
                                    {val}
            ...
            <p className="sc__week-table-footnote">
              ...
              Hours are grouped into two 12-hour sections: 00:00–11:00 and
              12:00–23:00. All 24 hours are included in totals.
            </p>
          </section>
        )}
      </div>
        ...
        >
          ...
          >
            ...
            <div className="sc-modal__tabs">
              <button
                type="button"
                className={`sc-modal__tab${
                  copyModalTab === 'single' ? ' sc-modal__tab--active' : ''
                }`}
              ...
              <button
                type="button"
                className={`sc-modal__tab${
                  copyModalTab === 'pattern' ? ' sc-modal__tab--active' : ''
                }`}
              ...
              {copyModalTab === 'single' ? (
                <>
                  ...
                  <div className="sc-modal__step">
                    <span className="sc-modal__step-num-title">
                      1. Copy From
                    </span>
                    <div
                    ...
                    >
                      ...
                      <span className="sc-modal__date-text">
                        {formatDateDisplay(modalCopyFrom)}
                      </span>
                  ...
                  <div className="sc-modal__step">
                    ...
                    <span className="sc-modal__step-subtitle">
                      Select Date Range
                    </span>
                    ...
                    <div className="sc-modal__range-row">
                      <div
                        ...
                        onClick={() =>
                          modalCopyToStartRef.current?.showPicker()
                        }
                      >
                        <span className="sc-modal__range-label">
                          Start Date
                        </span>
                        <span className="sc-modal__range-val">
                          {formatDateDisplay(modalCopyToStart)}
                        </span>
                      ...
                      >
                        ...
                        <span className="sc-modal__range-val">
                          {formatDateDisplay(modalCopyToEnd)}
                        </span>
                        <span className="sc-modal__range-icon">
                          <svg
                            width="14"
                  ...
                  <div className="sc-modal__info-box">
                    ...
                    <span className="sc-modal__info-text">
                      This will copy the configuration from{' '}
                      {formatDateDisplay(modalCopyFrom)} to {copyToDatesCount}{' '}
                      selected {copyToDatesCount === 1 ? 'date' : 'dates'}.
                    </span>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div className="sc-modal__step">
                    <span className="sc-modal__step-num-title">
                      1. Select Week to Copy
                    </span>
                    <span className="sc-modal__step-subtitle">
                      Pick any date — its full Mon–Sun week is used
                    </span>
                    <div
                      className="sc-modal__date-selector"
                    ...
                    >
                      <span className="sc-modal__date-icon">📅</span>
                      <span className="sc-modal__date-text">
                        {formatDateDisplay(patternWeekDates[0])} –{' '}
                        {formatDateDisplay(patternWeekEnd)}
                      </span>
                      <span className="sc-modal__chevron">▾</span>
                      <input
                        ...
                        onChange={(e) =>
                          handlePatternWeekChange(e.target.value)
                        }
                        className="sc-modal__date-hidden"
                  ...
                  <div className="sc-modal__step">
                    ...
                    <span className="sc-modal__step-subtitle">
                      Apply this week&apos;s pattern to every upcoming week
                      through:
                    </span>
                    <div
                      className="sc-modal__date-selector"
                    ...
                    >
                      <span className="sc-modal__date-icon">📅</span>
                      <span className="sc-modal__date-text">
                        {formatDateDisplay(modalPatternUntil)}
                      </span>
                      <span className="sc-modal__chevron">▾</span>
                      <input
                    ...
                    <div className="sc-modal__quick-weeks">
                      {PATTERN_QUICK_WEEKS.map((weeks) => (
                        <button
                          ...
                          onClick={() =>
                            setModalPatternUntil(
                              addDays(patternWeekEnd, weeks * 7)
                            )
                          }
                        >
                          {weeks} weeks
                  ...
                  <div className="sc-modal__info-box">
                    ...
                    <span className="sc-modal__info-text">
                      This will repeat the week of{' '}
                      {formatDateDisplay(patternWeekDates[0])} across the next{' '}
                      {patternTargetCount}{' '}
                      {patternTargetCount === 1 ? 'day' : 'days'}
                      {patternFullWeeks > 0
                        ? ` (${patternFullWeeks} full ${
                            patternFullWeeks === 1 ? 'week' : 'weeks'
                          }${
                            patternLeftoverDays > 0
                              ? ` + ${patternLeftoverDays} ${
                                  patternLeftoverDays === 1 ? 'day' : 'days'
                                }`
                              : ''
                          })`
                        : ''}
            ...
            <div className="sc-modal__footer">
              ...
              <button
                ...
                onClick={
                  copyModalTab === 'single'
                    ? handleModalApplyCopy
                    : handleApplyPatternCopy
                }
              >
                Copy Configuration
              </button>
